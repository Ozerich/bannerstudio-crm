            ...
            array('header' => 'Стоимость', 'htmlOptions' => array('class' => 'column-price'), 'type' => 'html', 'value' => function ($data, $row) {
               $result = '';
                ...
                foreach ($data->workers as $user) {
                    $result .= '<div class="user"><span class="username worker">'.$user->login.'</span><span class="price worker">'.$data->worker_price.' руб.</span>';
                }

                foreach ($data->customers as $user) {
                    $result .= '<div class="user"><span class="username customer">'.$user->login.'</span><span class="price customer">'.$data->customer_price.' руб.</span>';
                }
